...
multiple ammortized loans such as student loans or mortgagees to
...
section of the app. It takes a dataframe that consists of the dataframes
...
wanted to use accordians, the code for adding the loan information input
...
accordian fields for loan information is re-created. For each loan X:
...
-   If loan X is a new loan (not previously entered), create a new blank
    ...
    'delete' the information they had inputed.
-   Create an accordian object for loan X using the `accordianItem`
    function from the `shinydashboardPlus` library.

After the previous step, there is a list of accordian items that need to
...
the accordian list to insert multiple accordians into a single UI.
